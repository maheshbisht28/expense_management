...
class Add_expense_form(ModelForm):
	
	class Meta:
		model=Expense
		fields=["user","category","ex_name","date","money","desc"]
...
class show_expense_form(ModelForm):
...
class date_form(forms.Form):
	date_start=forms.DateField(widget=forms.DateInput(format=('%d-%m-%Y'),attrs={'class': 'datepicker', 'placeholder': 'Select a date', 'type': 'date'}),required=False)
	date_end=forms.DateField(widget=forms.DateInput(format=('%d-%m-%Y'),attrs={'class': 'datepicker', 'placeholder': 'Select a date', 'type': 'date'}),required=False)
	# forms.DateTimeField(widget = forms.SelectDateWidget)
